...
func (s *AdminPuskesmasService) ProfileUpdate(ctx context.Context, request *model.AdminPuskesmasProfileUpdateRequest) error {
	tx := s.DB.WithContext(ctx).Begin()
	defer tx.Rollback()
...
func (s *AdminPuskesmasService) PasswordUpdate(ctx context.Context, request *model.AdminPuskesmasPasswordUpdateRequest) error {
	tx := s.DB.WithContext(ctx).Begin()
	defer tx.Rollback()
